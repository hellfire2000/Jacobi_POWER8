...
void kernel_simd(Type dst[NZ+2*VS][NY+2*VS][NX+2*VS], Type src[NZ+2*VS][NY+2*VS][NX+2*VS])
{
		...
		for(k = VS; k < NZ+VS; k ++){
			for(j = VS; j < NY+VS; j ++){
				...
				//value assignment
					west   = vec_vsx_ld(-16, &src[k][j][i]);
					cur[0] = vec_vsx_ld(  0, &src[k][j][i]);
					cur[1] = vec_vsx_ld( 16, &src[k][j][i]);
					east   = vec_vsx_ld( 32, &src[k][j][i]);

					n2[0] = vec_vsx_ld(  0, &src[k][j+2][i]);
					n2[1] = vec_vsx_ld( 16, &src[k][j+2][i]);

					n1[0] = vec_vsx_ld(  0, &src[k][j+1][i]);
					n1[1] = vec_vsx_ld( 16, &src[k][j+1][i]);

					s2[0] = vec_vsx_ld(  0, &src[k][j-2][i]);
					s2[1] = vec_vsx_ld( 16, &src[k][j-2][i]);

					s1[0] = vec_vsx_ld(  0, &src[k][j-1][i]);
					s1[1] = vec_vsx_ld( 16, &src[k][j-1][i]);

					t2[0] = vec_vsx_ld(  0, &src[k+2][j][i]);
					t2[1] = vec_vsx_ld( 16, &src[k+2][j][i]);

					t1[0] = vec_vsx_ld(  0, &src[k+1][j][i]);
					t1[1] = vec_vsx_ld( 16, &src[k+1][j][i]);

					b2[0] = vec_vsx_ld(  0, &src[k-2][j][i]);
					b2[1] = vec_vsx_ld( 16, &src[k-2][j][i]);

					b1[0] = vec_vsx_ld(  0, &src[k-1][j][i]);
					b1[1] = vec_vsx_ld( 16, &src[k-1][j][i]);
				...
				//compute
...
#if 0
					west   = vec_xxpermdi(west,   cur[0], 1);
					cur[0] = vec_xxpermdi(cur[0], cur[1], 1);
					cur[1] = vec_xxpermdi(cur[1], east,   1);
#else
					west   = vec_xxpermdi(cur[0], west,   2);
					cur[0] = vec_xxpermdi(cur[1], cur[0], 2);
					cur[1] = vec_xxpermdi(east,   cur[1], 2);
#endif
					...
					vec_vsx_st(r0,  0, &dst[k][j][i]);
					vec_vsx_st(r1, 16, &dst[k][j][i]);
				}
#else
				for(i = VS; i < NX+VS; i++){
