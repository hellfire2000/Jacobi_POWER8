...
void kernel_simd(Type dst[NZ+2*VS][NY+2*VS][NX+2*VS], Type src[NZ+2*VS][NY+2*VS][NX+2*VS])
{
	...
	int bi, bj, bk;
	#pragma omp parallel for schedule(static) private(bi,bj,bk,i,j,k), shared(v04,v03,v02)

	for(bk = 0; bk < NZ; bk+=60){
		for(bj = 0; bj < NY; bj+=124){
			for(bi = 0; bi < NX; bi+=252){

				for(k = VS; k < 60+VS; k ++){
					for(j = VS; j < 124+VS; j ++){
#ifdef VSXOPT
						for(i = VS; i <252+VS; i+=4){
						//declaration
						...
						//value assignment
							west   = vec_xld2(-16, &src[bk+k][bj+j][bi+i]);
							cur[0] = vec_xld2(  0, &src[bk+k][bj+j][bi+i]);
							cur[1] = vec_xld2( 16, &src[bk+k][bj+j][bi+i]);
							east   = vec_xld2( 32, &src[bk+k][bj+j][bi+i]);

							n2[0] = vec_xld2(  0, &src[bk+k][bj+j+2][bi+i]);
							n2[1] = vec_xld2( 16, &src[bk+k][bj+j+2][bi+i]);

							n1[0] = vec_xld2(  0, &src[bk+k][bj+j+1][bi+i]);
							n1[1] = vec_xld2( 16, &src[bk+k][bj+j+1][bi+i]);

							s2[0] = vec_xld2(  0, &src[bk+k][bj+j-2][bi+i]);
							s2[1] = vec_xld2( 16, &src[bk+k][bj+j-2][bi+i]);

							s1[0] = vec_xld2(  0, &src[bk+k][bj+j-1][bi+i]);
							s1[1] = vec_xld2( 16, &src[bk+k][bj+j-1][bi+i]);

							t2[0] = vec_xld2(  0, &src[bk+k+2][bj+j][bi+i]);
							t2[1] = vec_xld2( 16, &src[bk+k+2][bj+j][bi+i]);

							t1[0] = vec_xld2(  0, &src[bk+k+1][bj+j][bi+i]);
							t1[1] = vec_xld2( 16, &src[bk+k+1][bj+j][bi+i]);

							b2[0] = vec_xld2(  0, &src[bk+k-2][bj+j][bi+i]);
							b2[1] = vec_xld2( 16, &src[bk+k-2][bj+j][bi+i]);
...
#else
						for(i = VS; i < NX+VS; i++){
							dst[bk+k][bj+j][bi+i] = 0.4*src[bk+k][bj+j][bi+i] + 0.3*(src[bk+k-1][bj+j][bi+i]+src[bk+k+1][bj+j][bi+i]+src[bk+k][bj+j-1][bi+i]+src[bk+k][bj+j+1][bi+i]+src[bk+k][bj+j][bi+i-1]+src[bk+k][bj+j][bi+i+1]) + 0.2*(src[bk+k-2][bj+j][bi+i]+src[bk+k+2][bj+j][bi+i]+src[bk+k][bj+j-2][bi+i]+src[bk+k][bj+j+2][bi+i]+src[bk+k][bj+j][bi+i-2]+src[bk+k][bj+j][bi+i+2]) ;
						}
#endif
					}
				}
