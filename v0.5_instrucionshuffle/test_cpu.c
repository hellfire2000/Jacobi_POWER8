...
void kernel_simd(Type dst[NZ+2*VS][NY+2*VS][NX+2*VS], Type src[NZ+2*VS][NY+2*VS][NX+2*VS])
{
    ...
    int bi,bj,bk;		
    //#pragma omp parallel for num_threads(THREADS_NUM)
    #pragma omp parallel for private(bi,bj,bk,i,j,k)
		for(bk = 0; bk < NZ; bk+=62)
			for(bj = 0; bj < NY; bj+=126)
        for(bi = 0; bi < NX; bi+=256)

		for(k = VS; k < 62+VS; k ++)
			for(j = VS; j <126+VS; j++)
        for(i = VS; i < 256+VS; i+=8){
        //output
          vector double r0, r1, r2, r3; 
        //source
          vector double n1[4], s1[4], t1[4], b1[4], cur[5];
        //temp
          vector double vtmp[4], vtmp2[4], vtmp3[4]; // short for vtmp[1][4]

        //value assignment
          cur[0] = vec_xld2(0, &src[bk+k][bj+j][bi+i-1]);
          cur[1] = vec_xld2(16, &src[bk+k][bj+j][bi+i-1]);
          cur[2] = vec_xld2(32, &src[bk+k][bj+j][bi+i-1]);
          cur[3] = vec_xld2(48, &src[bk+k][bj+j][bi+i-1]);
          cur[4] = vec_xld2(64, &src[bk+k][bj+j][bi+i-1]);
          ...
          vtmp3[0] = vec_add(cur[0], cur[1]);
          vtmp3[1] = vec_add(cur[1], cur[2]);
          vtmp3[2] = vec_add(cur[2], cur[3]);
          vtmp3[3] = vec_add(cur[3], cur[4]);
        ...
        //compute
          ...
          vtmp2[0] = vec_add(t1[0], b1[0]);
          vtmp2[1] = vec_add(t1[1], b1[1]);
          vtmp2[2] = vec_add(t1[2], b1[2]);
          vtmp2[3] = vec_add(t1[3], b1[3]);

          r0 = vec_madd(v03, vtmp3[0], r0);
          r1 = vec_madd(v03, vtmp3[1], r1);
          r2 = vec_madd(v03, vtmp3[2], r2);
          r3 = vec_madd(v03, vtmp3[3], r3);
          
          vtmp[0] = vec_add(vtmp[0], vtmp2[0]);
          vtmp[1] = vec_add(vtmp[1], vtmp2[1]);
          vtmp[2] = vec_add(vtmp[2], vtmp2[2]);
          vtmp[3] = vec_add(vtmp[3], vtmp2[3]);
