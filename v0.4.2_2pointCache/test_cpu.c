...
void kernel_simd(Type dst[NZ+2*VS][NY+2*VS][NX+2*VS], Type src[NZ+2*VS][NY+2*VS][NX+2*VS])
{
    ...
    #pragma omp parallel for private(bi,bj,bk,i,j,k)
		for(bk = 0; bk < NZ; bk+=32)
			for(bj = 0; bj < NY; bj+=32)
        for(bi = 0; bi < NX; bi+=2048)

		for(k = VS; k < 32+VS; k ++)
			for(j = VS; j < 32+VS; j++)
        for(i = VS; i < 2048+VS; i+=4){
        //output
          vector double r0, r1; 
        //source
          vector double n1[2], s1[2], t1[2], b1[2], cur[3];
        //temp
          vector double vtmp[2]; // short for vtmp[1][4]
