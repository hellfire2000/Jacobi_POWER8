...
void kernel_simd(Type dst[NZ+2*VS][NY+2*VS][NX+2*VS], Type src[NZ+2*VS][NY+2*VS][NX+2*VS])
{
    ...
    #pragma omp parallel for private(i,j,k), shared(v04, v03)
    //#pragma omp parallel for num_threads(THREADS_NUM)
		for(k = VS; k < NZ+VS; k ++)
			for(j = VS; j < NY+VS; j++)
        for(i = VS; i < NX+VS; i+=12){
        ...
        //value assignment
          cur[0] = vec_xld2( -8, &src[k][j][i]);
          cur[1] = vec_xld2(  8, &src[k][j][i]);
          cur[2] = vec_xld2( 24, &src[k][j][i]);
          cur[3] = vec_xld2( 40, &src[k][j][i]);
          cur[4] = vec_xld2( 56, &src[k][j][i]);
          cur[5] = vec_xld2( 72, &src[k][j][i]);
          cur[6] = vec_xld2( 88, &src[k][j][i]);

          n1[0] = vec_xld2(0, &src[k][j+1][i]);
          n1[1] = vec_xld2(16, &src[k][j+1][i]);
          n1[2] = vec_xld2(32, &src[k][j+1][i]);
          n1[3] = vec_xld2(48, &src[k][j+1][i]);
          n1[4] = vec_xld2(64, &src[k][j+1][i]);
          n1[5] = vec_xld2(80, &src[k][j+1][i]);
          ...
          s1[4] = vec_xld2(64, &src[k][j-1][i]);
          s1[5] = vec_xld2(80, &src[k][j-1][i]);
          ...
          t1[4] = vec_xld2(64, &src[k+1][j][i]);
          t1[5] = vec_xld2(80, &src[k+1][j][i]);
          ...
          b1[4] = vec_xld2(64, &src[k-1][j][i]);
          b1[5] = vec_xld2(80, &src[k-1][j][i]);
          ...
          vec_xstd2(r4, 64, &dst[k][j][i]);
          vec_xstd2(r5, 80, &dst[k][j][i]);
